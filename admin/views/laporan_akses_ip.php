<section class="content container-fluid">
  <div class="row-fluid">
      <div class="navbar">
          <div class="navbar-inner">
              <ul class="breadcrumb">
                 <h2>Halaman Tampil Akses Informasi Publik</h2>
                 <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#laporan">Tambah Laporan Akses Informasi Publik</button>
              </ul>
          </div>
      </div>
  </div>
  <table class="table table-hover table-striped" id="tabel_berita">
      ...
      <tbody>
          <?php  
              $no = 1;
              while($data = $tampilLaporanAksesIp->fetch_object()):
          ?>
          <tr>
              <td><?php echo $no++ ?></td>
              ...
              <td><button type="button" id="tombol_laporan_akses_ip" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#modal_edit_laporan_akses_ip" data-id="<?=$data->id;?>"><i class="fa fa-edit fa-fw"></i> Edit</button>
                <a href="<?=$data->link?>" class="btn btn-xs btn-success" target="_blank"><i class="fa fa-eye fa-fw"></i> Lihat File</a>
                <a href="?halaman=laporan_akses_ip&act=del&id=<?=$data->id;?>" class="btn btn-xs btn-danger" onclick="return confirm('Yakin Ingin Hapus Data Ini?')"><i class="fa fa-trash"></i> Hapus</a>
              </td>
          </tr>
          <?php  
              endwhile;
...
<div class="modal fade" id="modal_edit_laporan_akses_ip" role="dialog">
    <div class="modal-dialog">
        <div id="content-data_edit_laporan_akses_ip"></div>
    </div>
</div>
    ...
    <div class="modal-content">
      <form action="" method="post">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Tambah Laporan Tahunan</h4>
        </div>
        <div class="modal-body">
          <div class="column-full">
              <label for="judul">Jenis Laporan</label>
              <select  class="form-control" name="jenis_laporan" id="jenis_laporan" required>
                <option value="laporan_akses_ip">Laporan Akses Informasi Publik</option>
              </select>
          </div> 
          <div class="column-full">
...
<?php  
      ...
      if (!empty($link) && !empty($nama_laporan)) {
         ...
         if($masuk = 'masuk'){
          echo '<script>alert("Berhasil Tambah'.' '.$nama_laporan.'");
                window.location="?halaman=laporan_akses_ip";
          </script>';
         }
      }
  }
  ...
  if (@$_GET['act'] == 'del') {
    ...
    if ($hapus == 'terhapus') {
      ...
      echo '<script>alert("Agenda Gagal Dihapus");window.location="?halaman=laporan_akses_ip";</script>';
    }
  }
?>
