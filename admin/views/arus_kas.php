<section class="content container-fluid">
  <div class="row-fluid">
      <div class="navbar">
          <div class="navbar-inner">
              <ul class="breadcrumb">
                  <h2>Arus Kas</h2>
                  <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#input">Tambah Info Arus Kas</button>
              </ul>
          </div>
      </div>
  </div>
  <table class="table table-hover table-striped" id="tabel_berita" style="table-layout: fixed; width: 100%">
      ...
      <tbody>
          <?php  
              $no = 1;
              while($data = $tampilArusKas->fetch_object()):
          ?>
          <tr>
              <td><?php echo $no++ ?></td>
              ...
              <td>
                <button type="button" id="tombol_edit_arus_kas" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#modal_edit_arus_kas" data-id="<?=$data->id;?>"><i class="fa fa-edit fa-fw"></i> Edit</button>
                <a href="<?=$data->link;?>" class="btn btn-xs btn-success" target="_blank"><i class="fa fa-eye fa-fw"></i> Lihat File</a>
                <a href="?halaman=arus_kas&act=del&id=<?=$data->id;?>" class="btn btn-xs btn-danger" onclick="return confirm('Yakin Ingin Hapus Data Ini?')"><i class="fa fa-trash"></i> Hapus</a>
              </td>
          </tr>
          <?php  
              endwhile;
  ...
  <?php  
    ...
    if (isset($_POST['input'])) {
        ...
        if (!empty($tahun)) {
            ...
            $masuk = $keuangan->InsertArusKas($tahun, $link);

           if($masuk = 'masuk'){
            echo '<script>alert("Berahsil Arus Kas Tahun '.$tahun.'");
                  window.location="?halaman=arus_kas";
            </script>';
           }
        }
    }
...
<div class="modal fade" id="modal_edit_arus_kas" role="dialog">
    <div class="modal-dialog">
        <div id="content-data_edit_arus_kas"></div>
    </div>
</div>
