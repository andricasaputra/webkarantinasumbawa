<section class="content container-fluid">
  <div class="row-fluid">
      <div class="navbar">
          <div class="navbar-inner">
              <ul class="breadcrumb">
                 <h2>Halaman Tampil Laporan Kekayaan</h2>
                 <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#laporan">Tambah Laporan Kekayaan</button>
              </ul>
          </div>
      </div>
  </div>
  <table class="table table-hover table-striped" id="tabel_berita">
      ...
      <tbody>
          <?php  
              $no = 1;
              while($data = $tampilLaporanKekayaan->fetch_object()):
          ?>
          <tr>
              <td><?php echo $no++ ?></td>
              ...
              <td><button type="button" id="tombol_laporan_kekayaan" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#modal_edit_laporan_kekayaan" data-id="<?=$data->id;?>"><i class="fa fa-edit fa-fw"></i> Edit</button>
                <a href="<?=$data->link?>" class="btn btn-xs btn-success" target="_blank"><i class="fa fa-eye fa-fw"></i> Lihat File</a>
                <a href="?halaman=laporan_kekayaan&act=del&id=<?=$data->id;?>" class="btn btn-xs btn-danger" onclick="return confirm('Yakin Ingin Hapus Data Ini?')"><i class="fa fa-trash"></i> Hapus</a>
              </td>
          </tr>
          <?php  
              endwhile;
...
<div class="modal fade" id="modal_edit_laporan_kekayaan" role="dialog">
    <div class="modal-dialog">
        <div id="content-data_edit_laporan_kekayaan"></div>
    </div>
</div>
...
<?php  
      ...
      if (!empty($link) && !empty($nama_laporan)) {
         ...
         if($masuk = 'masuk'){
          echo '<script>alert("Berhasil Tambah'.' '.$nama_laporan.'");
                window.location="?halaman=laporan_kekayaan";
          </script>';
         }
      }
  }
  ...
  if (@$_GET['act'] == 'del') {
    ...
    $hapus = $keuangan->DeleteLaporanKekayaan($id);
    ...
    if ($hapus == 'terhapus') {
      ...
      echo '<script>alert("Agenda Gagal Dihapus");window.location="?halaman=laporan_kekayaan";</script>';
    }
  }
?>
